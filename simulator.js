/* ==========================================================================
   AD Soluções — Simulator Logic
   Fetch prices, calculate quotes, and capture simulator leads.
   ========================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 2. Dark Mode Toggle (Match app.js)
    // ==========================================
    ...
    // ==========================================
    // 4. Load pricing structure & build inputs
    // ==========================================
    // Hardcoded fallback/direct data to prevent CORS issues on file:// protocol
    priceData = {
      "operator": {
        "id": "amil",
        "name": "Amil",
    ...
    // Dynamic state of added lives: { ageRange: quantity }
    const livesGroup = {};
    ...
    const loadSimulatorData = () => {
        try {
            ...
            const ageSelect = document.getElementById('sim-age-select');
            ageSelect.innerHTML = '';
            ...
            samplePrices.forEach(priceObj => {
                ...
                option.textContent = `Faixa de ${ageRange} anos`;
    ...
    // Helper to render added lives list
    const renderAddedLivesList = () => {
        const listContainer = document.getElementById('added-lives-list');
        listContainer.innerHTML = '';

        const activeRanges = Object.keys(livesGroup).filter(r => livesGroup[r] > 0);

        if (activeRanges.length === 0) {
            listContainer.innerHTML = '<p id="no-lives-placeholder" style="font-style: italic; font-size: 0.9rem; color: var(--color-text-muted);">Nenhuma vida adicionada ainda.</p>';
            updateSummaryLabels();
            return;
        }

        activeRanges.forEach(range => {
            const qty = livesGroup[range];
            const item = document.createElement('div');
            item.style.display = 'flex';
            item.style.justifyContent = 'space-between';
            item.style.alignItems = 'center';
            item.style.background = 'var(--color-off-white)';
            item.style.padding = '0.75rem 1rem';
            item.style.borderRadius = 'var(--border-radius-md)';
            item.style.marginBottom = '0.5rem';
            item.style.border = '1px solid rgba(89, 0, 4, 0.08)';

            item.innerHTML = `
                <div style="font-size: 0.9rem;">
                    <strong>${qty}</strong> ${qty > 1 ? 'vidas' : 'vida'} na faixa <strong>${range}</strong>
                </div>
                <button type="button" class="btn-remove-range" data-range="${range}" style="background: none; border: none; color: #ff3b30; cursor: pointer; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 0.25rem;">
                    Remover
                </button>
            `;

            // Dark theme support style
            if (document.documentElement.getAttribute('data-theme') === 'dark') {
                item.style.background = '#110406';
                item.style.borderColor = 'rgba(174, 142, 121, 0.15)';
            }
        ...
        listContainer.querySelectorAll('.btn-remove-range').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const range = e.currentTarget.dataset.range;
                delete livesGroup[range];
                renderAddedLivesList();
            });
        });
    ...
    // Add age group handler
    const btnAddAgeGroup = document.getElementById('btn-add-age-group');
    if (btnAddAgeGroup) {
        btnAddAgeGroup.addEventListener('click', () => {
            const rangeSelect = document.getElementById('sim-age-select');
            const qtyInput = document.getElementById('sim-age-qty');
            
            const selectedRange = rangeSelect.value;
            const qtyToAdd = parseInt(qtyInput.value || 0, 10);

            if (qtyToAdd < 1) {
                alert('A quantidade deve ser de pelo menos 1 vida.');
                return;
            }

            // Add or increment
            livesGroup[selectedRange] = (livesGroup[selectedRange] || 0) + qtyToAdd;
    ...
    const updateSummaryLabels = () => {
        ...
        let totalLives = 0;
        Object.values(livesGroup).forEach(qty => {
            totalLives += qty;
        });
    ...
    document.addEventListener('input', (e) => {
        if (e.target.classList.contains('sim-select')) {
            updateSummaryLabels();
        }
    });
    ...
    if (leadForm) {
        leadForm.addEventListener('submit', (e) => {
            ...
            setTimeout(() => {
                console.log('Lead Simulador Capturado:', {
